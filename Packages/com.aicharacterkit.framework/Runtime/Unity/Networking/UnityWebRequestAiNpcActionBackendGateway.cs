...
{
    /// <summary>
    /// Sends V3 conversation and reset JSON to explicit loopback endpoints.
    /// </summary>
    public sealed class UnityWebRequestAiNpcActionBackendGateway
        : IAiNpcActionBackendGateway
    {
        ...
        /// <summary>
        /// Creates a local-only V3 gateway with a positive timeout.
        /// </summary>
        public UnityWebRequestAiNpcActionBackendGateway(
            string respondEndpoint,
            string resetEndpoint,
            int timeoutSeconds)
        {
            this.respondEndpoint = ParseLoopbackEndpoint(respondEndpoint, nameof(respondEndpoint));
            this.resetEndpoint = ParseLoopbackEndpoint(resetEndpoint, nameof(resetEndpoint));
            if (timeoutSeconds <= 0)
            {
        ...
        /// <summary>
        /// Posts one V3 conversation request and validates HTTP and contract branches.
        /// </summary>
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!AiNpcJsonCodecV3.TrySerializeRequest(requestEnvelope, out var json, out _))
            {
            ...
            using (var request = CreateRequest(respondEndpoint, json))
            {
                ...
                if (AiNpcJsonCodecV3.TryDeserializeResponse(
                        responseJson,
                        out var response,
                        out _))
                {
        ...
        /// <summary>
        /// Posts one V3 reset request and validates HTTP and acknowledgement branches.
        /// </summary>
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!AiNpcJsonCodecV3.TrySerializeResetRequest(requestEnvelope, out var json, out _))
            {
                throw CreateProtocolException();
            }
            ...
            using (var request = CreateRequest(resetEndpoint, json))
            {
                ...
                cancellationToken.ThrowIfCancellationRequested();
                var responseJson = request.downloadHandler?.text ?? string.Empty;
                if (AiNpcJsonCodecV3.TryDeserializeResetResponse(
                        responseJson,
                        out var response,
                        out _))
                {
        ...
        /// <summary>
        /// Creates one credential-free JSON POST to a selected V3 loopback endpoint.
        /// </summary>
        ...
        /// <summary>
        /// Restricts configured V3 endpoints to explicit HTTP(S) loopback addresses.
        /// </summary>
        ...
        private static void ValidateHttpBranch(long responseCode, string status)
        {
            ...
            if ((isSuccess && status != AiNpcContractV3.SuccessStatus)
                || (!isSuccess && status != AiNpcContractV3.ErrorStatus))
            {
                throw CreateProtocolException();
            }
        }
